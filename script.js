...
function showResult() {
  ...
  const targetRate = Math.min(100, Math.round((total / (questions.length * 5)) * 100));
  ...
  if (targetRate >= 80) {
    comment = "<strong>당신은 완벽한 'AI 오케스트레이터'입니다.</strong> 기술의 파도에 올라타 AI를 도구로 부리며, 복잡한 시스템을 설계하는 아키텍트의 자질이 충분합니다. 2026년의 시장은 당신과 같은 인재를 갈구할 것입니다.";
  } else if (targetRate >= 50) {
    comment = "<strong>안정적인 생존 궤도에 진입했습니다.</strong> AI와의 협업에 익숙해지고 있으며, 단순 구현 이상의 가치를 고민하기 시작했습니다. 조금 더 비즈니스 설계 관점을 보강한다면 대체 불가능한 개발자가 될 것입니다.";
  } else {
    comment = "<strong>변화가 필요한 시점입니다.</strong> 현재의 코딩 방식은 AI에 의해 빠르게 자동화되고 있습니다. 언어의 문법보다는 '왜 이 시스템이 필요한지'와 'AI를 어떻게 도구로 활용할지'에 집중해 보세요.";
  }
